...
- **v0.4.0（进行中 2026-07-20）**：UA 解析迁移 + JWT jti 精准单点踢出已完成（pkg/ua 包 + handler 接入 + ListLoginDevices 响应增强 + auth 包 18 个测试 + middleware JTI 注入测试，8 个测试包全 PASS）；后续：多级代理 + 全语言 SDK + 在线更新 + 数据备份恢复 + 监控告警 + 通知系统
