...
- **v0.4.0（进行中 2026-07-20）**：UA 解析迁移 + JWT jti 精准单点踢出 + 2FA backup_codes DB 持久化 + 登录失败日志结构化 slog + 全语言 SDK 扩展 已完成（5 项迁移全绿；11 个测试包全 PASS；新增 5 个 SDK：Go/Java/C#/C++/易语言，签名对齐测试从 3 语言扩展到 7 语言 + 易语言 Windows-only skip）；后续：多级代理 + 灰度发布 + 在线更新 + 数据备份恢复 + 监控告警 + 通知系统
