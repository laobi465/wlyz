...
- **v0.6.6（已完成 2026-07-21）**：后端 HTTP server 端口冲突自动 +1 重试（`app.port_auto_increment` 配置开关 + `APP_PORT_AUTO_INCREMENT` 环境变量；`net.Listen` 探测与 `srv.Serve` 解耦；仅对 `EADDRINUSE` 重试，默认 false 生产安全）；前端 Vite dev server 默认已具备此行为（`strictPort: false`）
