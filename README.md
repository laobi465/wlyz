...
[![Version](https://img.shields.io/badge/version-0.6.7-blue.svg)](docs/CHANGELOG.md)
...
## 当前版本（v0.6.7 管理员登录死机 P0 修复 + v0.6.6 并发安全加固 + v0.6.5 前端登录跳转 + v0.6.4 GORM datetime + v0.6.3 migration 030 + v0.6.2 dirty 迁移 完成）
...
| **专项** | 前端死循环 | 1 P0 | ✅ v0.6.7 | `scheduleRefresh` 无限异步递归修复：`_refreshing` 并发锁 + `expires_at` 合法性校验 + 最小延迟 30s 兜底 + 后端 sys_config 最小值校验（`jwt.access_ttl_seconds≥600`） |
