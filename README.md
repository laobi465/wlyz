...
### v0.8.0 去除多主题与暗黑模式（回归单一明亮主题）

v0.8.0 应产品要求去除多主题（light/dark/blue/purple/green/auto）与暗黑模式，回归单一明亮主题，降低维护成本与 UI 出错概率。

- **theme.ts store 简化**：`ThemeMode` 收窄为 `'light'`，移除 `resolvedMode`/`isDark` getter、`toggleLightDark()`、`matchMedia` 监听器；新增清理旧 localStorage 数据逻辑
- **themes.scss 大幅删减**：删除 dark/blue/purple/green/auto 主题变量（280→52 行），仅保留 `:root` 默认明亮主题
- **删除 ThemeSwitcher.vue 组件**：完全删除主题切换 UI
- **BasicLayout.vue 移除 ThemeSwitcher 引用**：顶栏不再显示主题切换器
- **main.ts 移除 dark css-vars 导入**：减少打包体积约 30KB
- **i18n 移除 theme 翻译**：zh-CN.ts 和 en-US.ts 移除 theme 翻译块
- **兼容性处理**：旧暗黑用户刷新页面后自动清理 localStorage + 移除 `html.dark` class，恢复明亮主题
...
详见 [v0.8.0 CHANGELOG](docs/CHANGELOG.md#080---2026-07-21去除多主题与暗黑模式回归单一明亮主题)。

### v0.7.0 前端管理员后台 UI 系统性修复（5 P0 + 14 P1）

v0.7.0 修复「检查下前端管理员后台的 UI 是否有错误」发现的所有 P0 + P1 问题：5 个 P0 + 14 个 P1（共 19 项关键修复），覆盖 14 个文件。

- **P0 主题图标名错误**：`apps/admin/src/stores/theme.ts` 的 `THEME_OPTIONS` 中 `'Water Cup'`/`'Magic Stick'` 不是 EP 图标的有效导出名 → `'WaterCup'`/`'MagicStick'`
- **P0 PlatformNoticeBanner 白底白字**：`.banner-label` 用 `background: currentColor` + `color:#fff` 导致 `currentColor` 解析为白色 → 改用 `rgba(255, 255, 255, 0.25)`
- **P0 Dashboard toFixed 崩溃 + 无 loading**：`stats.revenue_month.toFixed(2)` 在数据为 null 时抛 TypeError；页面加载期间无 loading 反馈 → 新增 `formatMoney()` 兜底 + `loading` ref + `v-loading` 遮罩
- **P0 Profile.vue labelPosition 非响应式**：computed 读 `window.innerWidth` 非响应式数据，resize 不重算 → 改用 `isMobile` ref + resize 监听
- **P1 主题切换重叠根因**：`BasicLayout.vue` 的 `.header-right` 无 `flex-wrap`、子元素无 `flex-shrink:0` → 窄屏时主题切换与用户菜单重叠
- **P1 ResponsiveTable size-change 双触发**：EP `el-pagination` 切换 page-size 时双触发 `loadList` → `_sizeChanging` 标志位抑制
- **P1 Settlements.vue net_amount.toFixed 崩溃**：复用 `formatMoney()` 兜底
- **P1 9 个管理员页面系统性修复**：防抖守卫、clearValidate、dialog 响应式宽度、filter.page 重置、formatDate Invalid Date 兜底（详见 CHANGELOG 修复矩阵）
...
详见 [v0.7.0 CHANGELOG](docs/CHANGELOG.md#070---2026-07-21前端管理员后台-ui-系统性修复5-p0--14-p1)。

### v0.6.9 性能优化（/admin/dashboard 加载卡很久 + 主题切换监听器累积）

v0.6.9 修复 v0.6.8 后用户反馈的两个问题：1) 进入 /admin/dashboard 卡很久；2) 主题切换组件可能因监听器累积导致 UI 异常。

- **症状 1**：访问 `/admin/dashboard` 后页面卡很久才加载完成
- **根因 1**：后端 `apps/server/internal/handler/admin_business.go` 的 `AdminDashboard` handler 存在 **21 次串行 DB 查询**，其中 **7 日收入趋势是循环 7 次单独 SUM 查询**（经典 N+1）：
  - 每次 `for i := 6; i >= 0; i--` 循环执行 `SELECT SUM(total_amount) WHERE paid_at >= dayStart AND paid_at < dayEnd`
  - 7 次串行查询 = 7 倍延迟，app_order 表数据量大时累计数秒
  - 加上其他 14 次统计查询（tenant/agent/app/card/order/settlement 各 2-3 次），总延迟可达 5-10 秒
- **修复 1**：7 日趋势 N+1 → 1 次 GROUP BY 查询
  - `SELECT DATE(paid_at) AS day, SUM(total_amount) WHERE paid_at >= 7天前 GROUP BY DATE(paid_at)`
  - 1 次查询拿到所有有订单的日期，内存中补齐无订单日期为 0
  - 查询次数从 21 降至 15，7 日趋势部分从 7 次 → 1 次（延迟降低 ~85%）
- **症状 2**：左上角可能显示重叠的主题切换（v0.6.8 前端死循环修复后残留的 UI 异常）
- **根因 2**：`apps/admin/src/stores/theme.ts` 的 `init()` 每次调用都 `matchMedia.addEventListener('change', handler)`，无去重。ThemeSwitcher 组件每次路由切换重新挂载都会调 `init()`，导致监听器累积（虽不致死循环，但可能触发 UI 渲染异常）
- **修复 2**：theme store 新增 `_mqlHandlerAdded` 幂等标志
  - 首次 `init()` 添加监听器并置 `_mqlHandlerAdded = true`
  - 后续 `init()` 调用检查标志位，已添加则跳过
  - 页面刷新后 store 重建，标志位重置，正常重新添加（不影响功能）
  - `persist.paths` 仍只持久化 `['mode']`，`_mqlHandlerAdded` 不持久化（符合预期）
...
详见 [v0.6.9 CHANGELOG](docs/CHANGELOG.md#069---2026-07-21admin-dashboard-加载卡很久--主题切换监听器累积修复)。
...
## 当前版本（v0.6.9 Dashboard 查询性能优化 + theme 监听器幂等 + v0.6.8 UpdateNotifier setInterval 累积卡死 P0 修复 + v0.6.7 管理员登录死机 P0 修复 + v0.6.6 并发安全加固 + v0.6.5 前端登录跳转 + v0.6.4 GORM datetime + v0.6.3 migration 030 + v0.6.2 dirty 迁移 完成）
...
| **Dashboard 查询性能优化 + theme 监听器幂等** | ✅ 已完成 v0.6.9 | **根因 1**：后端 `AdminDashboard` handler 21 次串行 DB 查询，其中 7 日收入趋势是循环 7 次单独 SUM 查询（经典 N+1），app_order 表数据量大时累计数秒导致 `/admin/dashboard` 卡很久；**修复 1** 7 日趋势 N+1 → 1 次 `GROUP BY DATE(paid_at)` 查询，内存补齐无订单日期为 0，查询次数 21→15，7 日趋势部分 7 次→1 次（延迟降低 ~85%）；**根因 2** `stores/theme.ts` `init()` 每次调用都 `matchMedia.addEventListener`，ThemeSwitcher 路由切换重新挂载会累积监听器，可能触发 UI 渲染异常；**修复 2** theme store 新增 `_mqlHandlerAdded` 幂等标志，首次 init 添加监听器并置 true，后续跳过，页面刷新后 store 重建正常重新添加（`persist.paths` 仍只持久化 `['mode']`） |
...
| **专项** | 查询性能 | 1 N+1 | ✅ v0.6.9 | `AdminDashboard` 7 日收入趋势 N+1 修复：7 次循环 `SUM WHERE paid_at >= dayStart AND paid_at < dayEnd` → 1 次 `GROUP BY DATE(paid_at)`，查询次数 21→15，7 日趋势部分延迟降低 ~85%；`theme.ts init()` matchMedia 监听器幂等：`_mqlHandlerAdded` 标志位防止 ThemeSwitcher 路由切换重新挂载累积监听器 |
...
- **v0.6.9（已完成 2026-07-21）**：**性能优化 —— /admin/dashboard 加载卡很久 + 主题切换监听器累积**。根因 1：后端 `AdminDashboard` handler 21 次串行 DB 查询，其中 7 日收入趋势是循环 7 次单独 SUM 查询（经典 N+1），app_order 表数据量大时累计数秒导致页面卡很久。修复 1：7 日趋势 N+1 → 1 次 `GROUP BY DATE(paid_at)` 查询，内存补齐无订单日期为 0，查询次数 21→15，7 日趋势部分 7 次→1 次（延迟降低 ~85%）。根因 2：`stores/theme.ts` `init()` 每次调用都 `matchMedia.addEventListener`，ThemeSwitcher 路由切换重新挂载会累积监听器，可能触发 UI 渲染异常（左上角主题切换重叠）。修复 2：theme store 新增 `_mqlHandlerAdded` 幂等标志，首次 init 添加监听器并置 true，后续跳过，页面刷新后 store 重建正常重新添加；后端 `go build ./internal/handler/` PASS + 前端 `npm run build` PASS（17.32s）
