...
- **v0.4.0（进行中 2026-07-20）**：UA 解析迁移 + JWT jti 精准单点踢出 + 2FA backup_codes DB 持久化 + 登录失败日志结构化 slog 已完成（4 项迁移全绿；10 个测试包全 PASS，含新增 internal/auth + internal/logger + internal/handler 共 37 个新测试）；后续：多级代理 + 全语言 SDK + 在线更新 + 数据备份恢复 + 监控告警 + 通知系统
