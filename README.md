...
[![Version](https://img.shields.io/badge/version-0.2.7-blue.svg)](docs/CHANGELOG.md)
...
| 代理核心页面（购卡/订单/佣金/提现） | ✅ 已完成 v0.2.5 | Dashboard + Cards + Orders + Commission + Balance 全响应式，含申请提现/充值对话框（后端接口待 v0.3.0 联调） |
| 三角色 Profile + 双 Dashboard | ✅ 已完成 v0.2.6 | admin/tenant/agent 账号设置（基础资料/改密/2FA/提现账户）+ admin/tenant 工作台（8 数据卡 + 趋势图 + 排行榜），全响应式 H5 |
| 前端三角色全页面响应式 H5 完整覆盖 | ✅ 已完成 v0.2.7 | 16 个 PlaceholderView 全部替换为真实页面（admin 7 + tenant 8 + agent 1），PlaceholderView 占位阶段彻底结束 |
| 后端业务接口（admin/tenant/agent dashboard + profile + CRUD） | ⏳ 计划中 v0.3.0 | 当前为 501 占位，待 v0.3.0 实现 dashboard/profile/2FA/登录设备/CRUD 等 |
...
- **v0.2.5**：代理核心页面（购卡/订单/佣金/提现，响应式 H5）
- **v0.2.6**：三角色 Profile + 双 Dashboard（响应式 H5）
- **v0.2.7**：全部剩余 16 个 PlaceholderView 替换为真实页面（响应式 H5 完整覆盖）
- **v0.3.0**：开发者自有易支付 + 代理注册付费 + 三级公告 + SDK（Python/Node/PHP）+ 代理后端业务接口 + admin/tenant/agent dashboard & profile & CRUD 后端实现
