...
[![Version](https://img.shields.io/badge/version-0.6.3-blue.svg)](docs/CHANGELOG.md)
...
### v0.6.3 重要修复（migration 030 列数不匹配 + v0.6.2 dirty 迁移恢复 + MySQL 8.0 兼容）

v0.6.3 在 v0.6.2 基础上追加修复 migration 030 (`030_v0.5.0_notify_webhook`) 的 `Error 1136 (Column count doesn't match value count)`：

- **根因**：`030_v0.5.0_notify_webhook.up.sql` 的 INSERT 声明 6 列，但每个 VALUES 元组写了 7 个值（在 `config_value` 与 `config_type` 之间多了一个空字符串 `''`）
- **修复**：全部 10 行 VALUES 元组移除多余空字符串字段，列顺序对齐 sys_config 表 schema
- **全量扫描**：Python 脚本扫描所有 `migrations/*.up.sql` 的 `INSERT INTO sys_config` 列数一致性，031/032/033 全部匹配，仅 030 一处 bug
- **v0.6.2 修复**（前置）：migration 015 重写为 INFORMATION_SCHEMA + PREPARE/EXECUTE 兼容方案；migrator.go 新增 `MIGRATION_REPAIR_DIRTY=true` 显式 dirty 恢复 + MySQL advisory lock 并发保护；`mysql:8.0` → `mysql:8.0.36` 固定小版本
...
详见 [v0.6.3 CHANGELOG](docs/CHANGELOG.md#063---2026-07-21migration-030-列数不匹配修复) 与 [v0.6.2 CHANGELOG](docs/CHANGELOG.md#062---2026-07-21dirty-迁移恢复--mysql-80-兼容修复)。
...
## 当前版本（v0.6.3 安全审计 + dirty 迁移恢复 + MySQL 8.0 兼容修复 + migration 030 列数修复完成）
