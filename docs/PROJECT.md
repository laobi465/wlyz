...
| S-08 | 通知模板 | ✅ | 短信/邮件/站内信三通道 + 模板 CRUD + 日志查询 + 失败重试 + 测试发送（v0.4.0：9 个 admin 端点 + 16 项 notify.* 配置 + Aliyun SMS 骨架 + SMTP Email 实现 + Render 用 strings.NewReplacer 防 SSTI） |
...
| D-06 | 用户管理 | ✅ | 终端用户列表 + 详情 + 状态/封禁（v0.4.0：4 个 admin 端点 AdminListEndUsers/AdminEndUserStats/AdminGetEndUser/AdminUpdateEndUserStatus + end_user 表 + 关联卡密查询） |
...
| 公告增强 + 数据统计 | `notice.content_format`（text/html） | v0.4.0 公告增强 + 数据统计看板（首次登录强制弹窗 admin/tenant/agent 三端 popup API + notice_read 幂等标记 + is_popup=true 已发布未读过滤 + 公告置顶 + 显眼标签 + 富文本编辑 notice.richtext.enabled/max_length 校验 + 验证趋势图按 result/action 维度聚合近 30 天 + 代理业绩排行联表 agent+sys_tenant+app_order 支持 total_amount/commission/net_amount/order_count 四种排序，migration 019 + 9 项 notice.*/stats.* sys_config） | v0.4.0 |
| 高级分析 | `user_behavior_profile`（tenant_id/app_id/end_user_id/stat_date/login_count/verify_count/heartbeat_count/bind_count/success_count/fail_count/banned_count/distinct_ip_count/distinct_device_count/first_active_at/last_active_at，唯一索引 (end_user_id, stat_date)）+ `card_usage_profile`（card_id 维度 + device_mismatch_count，唯一索引 (card_id, stat_date)）+ `user_risk_score`（user_type/user_id/risk_score/risk_level/event_count/high_freq_hits/geo_anomaly_hits/new_device_hits/abnormal_ua_hits/fail_rate_high/multi_ip_hits/multi_dev_hits/last_event_at/last_eval_at/banned/banned_reason/banned_at，唯一索引 (user_type, user_id)） | v0.6.0 高级分析三大模块（用户行为分析按日聚合 log_verify + 卡密使用画像含 device_mismatch + 风险评分累计 raw_score=Σ(hits×weight) + DecayScore 时间衰减 + 4 级分级 low/medium/high/critical + critical 自动封禁候选 + 24h 异常模式实时检测失败率>50%/多IP>=3/多设备>=5 + 卡密号脱敏前4+****+后4 + 聚合 Worker 默认 3600s 周期重算 + 16 个 admin 端点，migration 032 + 21 项 analysis.* sys_config） | v0.6.0 |

> migration 文件：`apps/server/migrations/` 共 32 套（001 ~ 032），由 `internal/migration/migrator.go` 在 `InitContainer` 阶段自动执行。
