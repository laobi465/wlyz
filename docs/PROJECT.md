...
- 通知系统（v0.4.0 短信/邮件/站内信三通道 + 模板引擎 + 服务商抽象 + 失败重试 + 限流，Aliyun SMS + SMTP Email 双实现）；v0.5.0 集成扩展批次：钉钉机器人 + 企业微信机器人 + Telegram Bot 三 webhook 通道
...
> migration 文件：`apps/server/migrations/` 共 19 套（001 ~ 019），由 `internal/migration/migrator.go` 在 `InitContainer` 阶段自动执行。
