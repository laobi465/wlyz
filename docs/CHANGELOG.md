...
## [0.6.6] - 2026-07-21（后端 HTTP server 端口冲突自动 +1 重试）

### [新增] 后端端口冲突自动 +1 重试

#### 背景

开发期多次启动后端实例时，8080 端口常被前一个未退出的进程占用，`srv.ListenAndServe()` 直接 `log.Fatalf` 退出，需手动改端口或杀进程。前端 Vite dev server 默认 `strictPort: false` 已自动 +1，但后端 Go server 无此能力。

#### 实现

**设计原则**：端口探测与 HTTP serve 解耦——先用 `net.Listen` 找可用端口，再 `srv.Serve(listener)`。

**1. 新增配置项**（`apps/server/internal/config/config.go`）：`AppConfig` 新增 `PortAutoIncrement bool`（默认 false 生产安全）+ `PortMaxAttempts int`（默认 20），支持环境变量 `APP_PORT_AUTO_INCREMENT` / `APP_PORT_MAX_ATTEMPTS`。

**2. 启动逻辑改造**（`apps/server/cmd/main.go`）：`ListenAndServe` → `net.Listen` 探测 + `srv.Serve(listener)`；新增 `listenWithPortRetry` + `isAddrInUse` 辅助函数。

**3. 重试策略**：只对 `EADDRINUSE`（端口占用）重试，其他错误（权限不足等）直接返回；端口越界保护（>65535 停止）。

#### 为什么默认 false

生产环境端口由 nginx/反代锁定，自动漂移会导致反代失配。开发期通过 `APP_PORT_AUTO_INCREMENT=true` 或 config.yaml 显式开启。

#### 铁律遵循

- 04 禁硬编码：最大尝试次数走配置
- 05 配置后台化：开关与次数从 config.yaml / 环境变量读取
- 06 防幻觉：只对 `EADDRINUSE` 重试，不盲目重试所有错误

#### 涉及文件

- `apps/server/internal/config/config.go`：新增 `PortAutoIncrement` / `PortMaxAttempts` 字段 + 默认值 + 环境变量覆盖
- `apps/server/cmd/main.go`：启动逻辑改造 + 新增 `listenWithPortRetry` / `isAddrInUse`
- `configs/config.yaml.example`：新增配置样例与注释

#### 使用方式

```bash
# 开发期（环境变量）
APP_PORT_AUTO_INCREMENT=true go run ./apps/server/cmd/
...
1. **`homePath` getter 在 role 为空时产生 `//dashboard`** → Vue Router 规范化为 `/dashboard` → 404
2. **路由守卫在 stale state 下未强制 logout**：accessToken 存在但 role 为空时，跳转 `//dashboard` → 404
3. **`login/index.vue` 的 `validate` 使用 callback 风格**：`await formRef.value.validate(async (valid) => {...})` 中 await 是 no-op（Element Plus validate 传 callback 返回 undefined）
4. **前端 UI Tab 角色与后端实际角色不一致时未以后端为准**：应以后端 `resp.user.role` 为权威来源
5. **redirect 参数未做白名单校验**：open redirect / XSS 风险

#### 修复（5 项）

1. `apps/admin/src/stores/auth.ts`：`homePath` getter 兜底 role 为空时返回 `/login`
2. `apps/admin/src/router/index.ts`：守卫新增 stale state 检测（已登录但 role 为空时强制 logout）
3. `apps/admin/src/views/login/index.vue`：改用 Promise 风格 `validate()`
4. `apps/admin/src/views/login/index.vue`：优先使用后端返回的 role（`serverRole`），兜底 UI Tab
5. `apps/admin/src/views/login/index.vue`：redirect 白名单校验（仅允许 `/admin` `/tenant` `/agent` 开头）

#### 铁律遵循

- 04 禁硬编码：`validRoles` 从 `UserRole` 类型派生
- 05 配置后台化：role 来源于后端 `resp.user.role`
- 06 防幻觉：后端 role 为权威来源，redirect 显式白名单校验

#### 操作命令

升级后需清除浏览器 localStorage（旧持久化数据可能 role 为空）：
```javascript
