...
- [ ] [待开始] /api/v1/client/login 接口实现 - v0.2.0
- [ ] [待开始] /api/v1/client/verify 接口实现 - v0.2.0
- [ ] [待开始] /api/v1/client/heartbeat 接口实现 - v0.2.0
- [ ] [待开始] /api/v1/client/bind 接口实现 - v0.2.0
- [ ] [待开始] /api/v1/client/unbind 接口实现 - v0.2.0
- [ ] [待开始] /api/v1/client/logout 接口实现 - v0.2.0
...
| v0.2.0 | 待定 | [进行中] 一期 MVP 骨架阶段完成，业务逻辑实现中 |
| v0.3.0 | 待定 | [待开始] 二期增值 |
| v0.4.0 | 待定 | [待开始] 三期商业化 |
...
- **总任务数**：约 75 项
- **已完成**：22 项（骨架 + 部分中间件/工具/部署）
- **进行中**：1 项（认证模块作为下一阶段重点）
- **待开始**：约 52 项

### 已完成项汇总（22 项）

**项目骨架（7/8）**：Go 结构、Vue3 三布局、docker-compose、Dockerfile、宝塔脚本、.env、配置加载
**数据库（2/3）**：26 张表 migration、seed 数据
**认证与多租户（2/7）**：多租户中间件骨架、bcrypt 工具
**卡密体系（1/5）**：卡密生成算法
**客户端验证 API（4/12）**：HMAC 签名中间件、Nonce 防重放、Timestamp 校验、RSA 响应签名工具
**安全防护（5/5）**：Nginx 限流、IP 黑名单、自动封禁、安全头、HTTPS 跳转
**部署运维（3/4）**：健康检查、Docker healthcheck、部署文档
**其他（3 项）**：日志分区表结构、代理注册页骨架、公告显示组件
