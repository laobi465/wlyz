# 待完成文档 (TODO / Backlog)

格式：`优先级 + 状态 + 条目 + 预计版本 + 备注`

- 优先级：`P0`（紧急）/ `P1`（高）/ `P2`（中）/ `P3`（低）
- 状态：`[待开始]` `[进行中]` `[已阻塞]` `[已延期]` `[已完成]`

---

## P0 紧急（一期 MVP 必须）

### [P0] 一期 MVP 核心闭环

#### 项目骨架搭建
- [ ] [待开始] 初始化 Go 项目结构（cmd/internal/pkg/migrations） - v0.2.0
- [ ] [待开始] 初始化 Vue3 admin 项目（超管/开发者/代理三布局） - v0.2.0
- [ ] [待开始] 初始化 Vue3 H5 项目（终端用户） - v0.2.0
- [ ] [待开始] 编写 docker-compose.yml（mysql/redis/api/admin/h5/nginx） - v0.2.0
- [ ] [待开始] 编写 Dockerfile（多阶段构建，非 root 运行） - v0.2.0
- [ ] [待开始] 编写宝塔面板安装脚本 baota-install.sh - v0.2.0
- [ ] [待开始] 编写 .env.example 和配置加载逻辑 - v0.2.0
- [ ] [待开始] 生成 RSA-4096 密钥对工具脚本 - v0.2.0

#### 数据库初始化
- [ ] [待开始] 编写全部 26 张表的 migration 文件 - v0.2.0
- [ ] [待开始] 编写 seed 数据（超管账号、默认套餐、默认 sys_config） - v0.2.0
- [ ] [待开始] 实现 golang-migrate 自动迁移机制 - v0.2.0

#### 认证与多租户
- [ ] [待开始] 平台超管登录 + JWT - v0.2.0
- [ ] [待开始] 开发者注册/登录/2FA - v0.2.0
- [ ] [待开始] 代理登录 + JWT - v0.2.0
- [ ] [待开始] 多租户隔离中间件（自动注入 tenant_id） - v0.2.0
- [ ] [待开始] 套餐配额检查中间件 - v0.2.0
- [ ] [待开始] 密码 bcrypt (cost=12) - v0.2.0
- [ ] [待开始] JWT Token 刷新机制 - v0.2.0

#### 应用管理
- [ ] [待开始] 应用 CRUD API - v0.2.0
- [ ] [待开始] AppKey/AppSecret/SignSecret 生成 - v0.2.0
- [ ] [待开始] 密钥轮换（保留旧密钥 7 天） - v0.2.0
- [ ] [待开始] 应用配置（一机一卡/心跳/宽限/解绑扣时） - v0.2.0
- [ ] [待开始] 应用列表/详情前端页面 - v0.2.0

#### 卡密体系
- [ ] [待开始] 卡类套餐 CRUD - v0.2.0
- [ ] [待开始] 卡密批量生成（SecureRandom + SHA-512 校验位） - v0.2.0
- [ ] [待开始] 卡密查询/封禁/解封/删除 - v0.2.0
- [ ] [待开始] 卡密导入导出 CSV - v0.2.0
- [ ] [待开始] 卡密状态机（unused/active/expired/banned/disabled） - v0.2.0
- [ ] [待开始] 卡密批量生成前端页面（含弹窗） - v0.2.0

#### 设备绑定
- [ ] [待开始] 设备指纹生成算法（CPU+主板+MAC+磁盘哈希） - v0.2.0
- [ ] [待开始] 一机一卡密绑定逻辑 - v0.2.0
- [ ] [待开始] 设备解绑扣时逻辑 - v0.2.0
- [ ] [待开始] 设备强制下线 - v0.2.0
- [ ] [待开始] 设备列表/封禁前端页面 - v0.2.0

#### 客户端验证 API
- [ ] [待开始] HMAC-SHA256 签名中间件 - v0.2.0
- [ ] [待开始] Nonce 防重放（Redis） - v0.2.0
- [ ] [待开始] Timestamp ±5 分钟校验 - v0.2.0
- [ ] [待开始] /api/v1/client/login 接口 - v0.2.0
- [ ] [待开始] /api/v1/client/verify 接口 - v0.2.0
- [ ] [待开始] /api/v1/client/heartbeat 接口 - v0.2.0
- [ ] [待开始] /api/v1/client/bind 接口 - v0.2.0
- [ ] [待开始] /api/v1/client/unbind 接口 - v0.2.0
- [ ] [待开始] /api/v1/client/logout 接口 - v0.2.0
- [ ] [待开始] RSA-4096 响应签名 - v0.2.0
- [ ] [待开始] 心跳保活 Redis Sorted Set - v0.2.0
- [ ] [待开始] 离线宽限期判定 - v0.2.0

#### 平台总支付
- [ ] [待开始] 超管后台支付配置页（S-06） - v0.2.0
- [ ] [待开始] 彩虹易支付下单接口 - v0.2.0
- [ ] [待开始] 彩虹易支付异步回调接口 - v0.2.0
- [ ] [待开始] 彩虹易支付同步跳转接口 - v0.2.0
- [ ] [待开始] 支付成功自动发卡 - v0.2.0
- [ ] [待开始] 支付回调防重入（Redis 锁） - v0.2.0
- [ ] [待开始] 30 分钟订单超时关闭 - v0.2.0
- [ ] [待开始] 平台抽成计算与结算记录 - v0.2.0

#### 管理后台基础页面
- [ ] [待开始] 超管后台：登录 + 看板 + 租户管理 + 套餐管理 + 系统配置 - v0.2.0
- [ ] [待开始] 开发者控制台：登录 + 工作台 + 应用管理 + 卡密管理 + 设备管理 - v0.2.0
- [ ] [待开始] 代理控制台：登录 + 工作台 + 卡密库存 + 卡密管理 - v0.2.0

#### 终端用户 H5
- [ ] [待开始] 首页 + 应用详情 + 购卡结算 + 支付结果 - v0.2.0
- [ ] [待开始] 查卡页 + 在线激活页 - v0.2.0
- [ ] [待开始] 用户登录/注册 - v0.2.0

#### 安全防护（基础）
- [ ] [待开始] Nginx 限流配置（100r/s + 敏感接口 10r/min） - v0.2.0
- [ ] [待开始] IP 黑名单手动管理 - v0.2.0
- [ ] [待开始] 卡密错误 5 次封 IP 1h - v0.2.0
- [ ] [待开始] 安全响应头配置 - v0.2.0
- [ ] [待开始] HTTPS 强制跳转 - v0.2.0

#### 部署与运维
- [ ] [待开始] /health 健康检查接口 - v0.2.0
- [ ] [待开始] Docker healthcheck - v0.2.0
- [ ] [待开始] 部署文档（Docker Compose + 宝塔两种方式） - v0.2.0
- [ ] [待开始] 安装向导页面（/install） - v0.2.0

---

## P1 高（二期增值）

### [P1] 二期增值功能

#### 开发者自有易支付
- [ ] [待开始] 套餐 allow_custom_pay 字段生效 - v0.3.0
- [ ] [待开始] 开发者支付配置页（D-18） - v0.3.0
- [ ] [待开始] tenant_pay_config 表读写 - v0.3.0
- [ ] [待开始] 双层支付模式切换逻辑 - v0.3.0
- [ ] [待开始] 开发者自有支付下单/回调接口 - v0.3.0
- [ ] [待开始] 开发自有支付附加月费订单 - v0.3.0
- [ ] [待开始] 切换支付方式时通知所有代理（站内信+横幅+弹窗） - v0.3.0

#### 代理注册付费流程
- [ ] [待开始] 开发者生成邀请码（D-08） - v0.3.0
- [ ] [待开始] 邀请码 CRUD + 状态机（active/disabled/exhausted/expired） - v0.3.0
- [ ] [待开始] 代理注册页（REG-01，3 步流程） - v0.3.0
- [ ] [待开始] 代理注册费支付（走平台总支付） - v0.3.0
- [ ] [待开始] 代理账号自动创建 + 关联开发者 - v0.3.0
- [ ] [待开始] 超管后台代理注册管理（S-17） - v0.3.0

#### 代理购买卡密
- [ ] [待开始] 代理充值申请（P-09） - v0.3.0
- [ ] [待开始] 开发者审核充值（D-19） - v0.3.0
- [ ] [待开始] 代理余额扣款生成卡密 - v0.3.0
- [ ] [待开始] 代理实时扫码购卡（P-10，备用） - v0.3.0
- [ ] [待开始] 代理佣金计算（percentage / diff 两种模式） - v0.3.0
- [ ] [待开始] 代理提现申请（P-05） - v0.3.0
- [ ] [待开始] 开发者审核提现 + 打款（D-14） - v0.3.0
- [ ] [待开始] 代理独立门户（P-06，仅展示，收款走开发者） - v0.3.0
- [ ] [待开始] 代理子域名绑定 - v0.3.0

#### 三级公告体系
- [ ] [待开始] 统一公告表 notice 读写 - v0.3.0
- [ ] [待开始] notice_target 精准投递 - v0.3.0
- [ ] [待开始] notice_read 已读记录 - v0.3.0
- [ ] [待开始] 平台总公告管理（S-15，富文本编辑） - v0.3.0
- [ ] [待开始] 开发者公告管理（S-16） - v0.3.0
- [ ] [待开始] 应用公告管理（D-10） - v0.3.0
- [ ] [待开始] 代理通知（type=agent_notify）自动写入 - v0.3.0
- [ ] [待开始] 开发者控制台顶部公告显示区（平台+开发者同时显示） - v0.3.0
- [ ] [待开始] 代理控制台公告中心（P-08） - v0.3.0
- [ ] [待开始] 首次登录强制弹窗 - v0.3.0
- [ ] [待开始] 公告置顶 + 显眼标签 - v0.3.0

#### 云变量与版本管理
- [ ] [待开始] 云变量 CRUD - v0.3.0
- [ ] [待开始] /api/v1/client/get_var 接口 - v0.3.0
- [ ] [待开始] 版本管理 CRUD - v0.3.0
- [ ] [待开始] /api/v1/client/version 接口（强制更新检查） - v0.3.0
- [ ] [待开始] /api/v1/client/notice 接口 - v0.3.0

#### 数据统计看板
- [ ] [待开始] 开发者工作台数据看板（卡密数/在线数/销量/收入） - v0.3.0
- [ ] [待开始] 验证趋势图（近 30 天） - v0.3.0
- [ ] [待开始] 应用销量 TOP 排行 - v0.3.0
- [ ] [待开始] 代理业绩排行 - v0.3.0
- [ ] [待开始] 超管平台看板（S-01） - v0.3.0

#### 客户端 SDK（3 语言）
- [ ] [待开始] Python SDK（keyauth-py） - v0.3.0
- [ ] [待开始] Node.js SDK（keyauth-node） - v0.3.0
- [ ] [待开始] PHP SDK（keyauth-php） - v0.3.0
- [ ] [待开始] SDK 文档与示例 - v0.3.0

#### 日志系统
- [ ] [待开始] 验证日志写入（异步） - v0.3.0
- [ ] [待开始] 操作日志写入（AOP 切面） - v0.3.0
- [ ] [待开始] 日志检索与导出 - v0.3.0
- [ ] [待开始] 日志按月分区自动维护 - v0.3.0

---

## P2 中（三期商业化）

### [P2] 三期商业化完整版

#### 多级代理
- [ ] [待开始] 二级代理支持 - v0.4.0
- [ ] [待开始] 三级代理支持 - v0.4.0
- [ ] [待开始] 跨级佣金结算 - v0.4.0

#### 全语言 SDK
- [ ] [待开始] Java SDK（含 Android） - v0.4.0
- [ ] [待开始] C# SDK（.NET 全平台） - v0.4.0
- [ ] [待开始] Go SDK - v0.4.0
- [ ] [待开始] C/C++ SDK（含 JNI） - v0.4.0
- [ ] [待开始] 易语言模块 - v0.4.0

#### 高级安全
- [ ] [待开始] 异地登录告警 - v0.4.0
- [ ] [待开始] 风控规则引擎 - v0.4.0
- [ ] [待开始] 设备指纹升级（多维度） - v0.4.0
- [ ] [待开始] Cloudflare WAF 集成 - v0.4.0

#### 灰度发布
- [ ] [待开始] 应用版本灰度推送 - v0.4.0
- [ ] [待开始] 灰度规则配置（按地区/比例） - v0.4.0

#### 数据备份与恢复
- [ ] [待开始] 数据库自动备份（每日全量+每小时增量） - v0.4.0
- [ ] [待开始] 一键恢复面板 - v0.4.0
- [ ] [待开始] 备份文件下载 - v0.4.0

#### 在线更新系统
- [ ] [待开始] Webhook 接收 GitHub Push - v0.4.0
- [ ] [待开始] 自动拉取构建重启 - v0.4.0
- [ ] [待开始] 后台更新管理面板（S-13） - v0.4.0
- [ ] [待开始] 管理员弹窗通知 - v0.4.0
- [ ] [待开始] 版本回滚 - v0.4.0
- 详见 references/11-github-auto-update.md

#### API 开放平台
- [ ] [待开始] 第三方接入授权 - v0.4.0
- [ ] [待开始] Webhook 事件推送 - v0.4.0
- [ ] [待开始] 开发者 API Token 管理 - v0.4.0

#### 监控告警
- [ ] [待开始] Prometheus + Grafana 集成 - v0.4.0
- [ ] [待开始] 异常 QPS 告警 - v0.4.0
- [ ] [待开始] 错误率 > 1% 告警 - v0.4.0
- [ ] [待开始] CPU/磁盘阈值告警 - v0.4.0

#### 通知系统
- [ ] [待开始] 短信模板（阿里云/腾讯云） - v0.4.0
- [ ] [待开始] 邮件模板（SMTP） - v0.4.0
- [ ] [待开始] 站内信 - v0.4.0

---

## P3 低（优化与扩展）

### [P3] 优化与扩展

#### 性能优化
- [ ] [待开始] MySQL 读写分离 - v0.5.0
- [ ] [待开始] API 水平扩展（无状态化） - v0.5.0
- [ ] [待开始] 卡密生成性能优化（目标 10000 条/秒） - v0.5.0
- [ ] [待开始] Redis 集群模式 - v0.5.0

#### 用户体验
- [ ] [待开始] 后台多主题切换 - v0.5.0
- [ ] [待开始] 暗黑模式（可选主题） - v0.5.0
- [ ] [待开始] 移动端响应式优化 - v0.5.0
- [ ] [待开始] 数据看板数字滚动动效 - v0.5.0

#### 国际化
- [ ] [待开始] 后台 i18n（中/英） - v0.5.0
- [ ] [待开始] SDK 多语言文档 - v0.5.0

#### 集成扩展
- [ ] [待开始] 独角数卡对接 - v0.5.0
- [ ] [待开始] 蓝米发卡对接 - v0.5.0
- [ ] [待开始] USDT/加密货币支付 - v0.5.0
- [ ] [待开始] 钉钉/企业微信通知 - v0.5.0

#### 主题市场
- [ ] [待开始] 多套主题模板 - v0.6.0
- [ ] [待开始] 主题编辑器 - v0.6.0

#### 高级分析
- [ ] [待开始] 用户行为分析 - v0.6.0
- [ ] [待开始] 卡密使用画像 - v0.6.0
- [ ] [待开始] 风险用户识别 - v0.6.0

---

## 已阻塞项

（暂无）

---

## 已延期项

（暂无）

---

## 任务依赖关系

```
一期 MVP（v0.2.0）
├─ 项目骨架 ──► 数据库迁移 ──► 认证模块 ──► 多租户中间件
│                                          │
│                                          ├─► 应用管理 ──► 卡密管理 ──► 设备绑定
│                                          │                          │
│                                          │                          ▼
│                                          │              客户端验证 API
│                                          │
│                                          └─► 平台总支付 ──► 自动发卡
│
└─ 前端三套 ──► 联调测试 ──► Docker 部署 ──► 文档

二期（v0.3.0）
├─ 开发者自有支付 ──► 双层切换 ──► 通知代理
├─ 代理注册付费 ──► 邀请码体系
├─ 代理购买卡密 ──► 余额系统 ──► 佣金结算 ──► 提现
├─ 三级公告 ──► 精准投递
├─ 云变量/版本 ──► 客户端接口扩展
└─ SDK（3 语言）

三期（v0.4.0）
├─ 多级代理
├─ 全语言 SDK
├─ 在线更新 ──► Webhook
└─ 监控告警
```

---

## 里程碑

| 版本 | 目标完成日期 | 状态 |
|---|---|---|
| v0.1.0 | 2026-07-19 | ✅ 已完成（规划） |
| v0.2.0 | 待定 | [待开始] 一期 MVP |
| v0.3.0 | 待定 | [待开始] 二期增值 |
| v0.4.0 | 待定 | [待开始] 三期商业化 |

---

**文档版本**：0.1.0  
**最后更新**：2026-07-19  
**维护者**：KeyAuth SaaS Team
