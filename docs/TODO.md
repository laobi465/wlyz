...
- **总任务数**：约 75 项
- **已完成**：约 45 项（v0.2.0 骨架 22 项 + v0.2.1 认证 8 项 + v0.2.2 应用/卡密/客户端 15 项）
- **进行中**：1 项（套餐配额检查中间件）
- **待开始**：约 29 项
...
**P0 紧急**：平台总支付（7 项：下单/回调/同步跳转/自动发卡/防重入/订单超时/抽成计算）、管理后台基础页面（3 项）、终端用户 H5（3 项）、套餐配额检查中间件、RSA 密钥对工具脚本、golang-migrate 自动迁移、卡密 CSV 导入导出、设备强制下线、安装向导页面
