...
#### 代理核心页面（响应式 H5） ⏳ v0.2.5 进行中
- [ ] [进行中] 代理 API 模块 `api/agent.ts`（dashboard/cards/generate/commission/withdraw/orders/me） - v0.2.5
- [ ] [进行中] 代理概览 `views/agent/Dashboard.vue`（余额/累计佣金/可提现/数据卡） - v0.2.5
- [ ] [进行中] 代理购卡 `views/agent/Cards.vue`（授权卡类 + 扣余额生成 + 结果展示） - v0.2.5
- [ ] [进行中] 代理订单 `views/agent/Orders.vue`（我的订单列表 + 状态筛选） - v0.2.5
- [ ] [进行中] 代理佣金 `views/agent/Commission.vue`（佣金明细 + 提现申请对话框） - v0.2.5
- [ ] [待核实] 后端 agent 路由现仅占位（501 待实现），待 v0.3.0 完成对端接口 - v0.2.5
...
| v0.2.5 | 2026-07-19 | [进行中] 代理核心页面（购卡/订单/佣金，响应式） |
| v0.3.0 | 待定 | [待开始] 二期增值 |
| v0.4.0 | 待定 | [待开始] 三期商业化 |
