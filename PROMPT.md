...
**v0.4.0 进行中**：UA 解析迁移 + JWT jti 精准单点踢出 + 2FA backup_codes DB 持久化 + 登录失败日志结构化 slog + 全语言 SDK 扩展 + 多级代理体系 + 灰度发布体系 + 在线更新体系 + 数据备份恢复体系 + 监控告警体系 已完成（10 项迁移全绿；`pkg/ua` + `internal/auth` + `internal/logger` + `internal/handler/profile_2fa_test.go` + 5 个新 SDK + `internal/multilevel`（27 个测试）+ `internal/grayscale`（33 个测试）+ `internal/update`（37 个测试）+ `internal/backup` + `internal/monitor`（53 个测试）共 180+ 个新测试，15 个测试包全 PASS）。
